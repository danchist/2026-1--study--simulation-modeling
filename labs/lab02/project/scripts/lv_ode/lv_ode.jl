...
function compute_fft(signal, dt)
    n = length(signal)
    spectrum = abs.(rfft(signal)) # Используем rfft для вещественных сигналов (возвращает только положительные частоты)
    freq = rfftfreq(n, 1/dt) # Соответствующие частоты для rfft
...
if true  # Установите в true для выполнения анализа
